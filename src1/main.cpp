...
int main()
{
	...
	try {
		...
		std::vector<std::vector<int> > numin;
		deltamin.resize(n);
		gamma.resize(n);
		numin.resize(n);
		for (int i = 0; i < n; ++i) {
		...
		for (int a = 0; a < n; ++a) {
			for (int b = a+2; b < n; ++b)
				numin[a][b] = Numin(points, a, b, delta0);
		}
		...
		fhtml << "  <h3>Минимальное количество аномальных точек при пороге <i>&delta;</i><sub>0</sub></h3>\n";
		if (n >= 3) {
			fhtml << "  <table border=\"1\" cellpadding=\"2\" cellspacing=\"0\" class=\"text\">\n";
			for (int a = 0; a < n-2; ++a) {
				fhtml << "  <tr>\n";
